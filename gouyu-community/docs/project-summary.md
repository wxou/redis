...
`PostCommentController` 当前只有领域入口，没有声明可调用方法，因此不计入 32 个可调用路由。
...
登录已恢复验证码校验：验证码不存在、过期或不匹配都会失败，登录成功后立即删除验证码，避免重复使用。`LoginRequest` 虽保留 `password` 字段以兼容旧请求结构，但后端会明确拒绝密码登录；`login2.html` 也直接提示当前只支持验证码登录，不再制造“密码已验证”的假象。
...
- **质量闭环**：编译、打包、页面、Nginx、MySQL、Redis 和 42 场景联调；
...
从“项目包装与独立交付”的角度看，当前改造目标已经完成；从“生产系统”的角度看，项目仍处在可运行原型和高质量教学工程之间，短信接入、细粒度权限、跨存储一致性、Stream 运维、评论与运营后台、自动化测试和部署体系是下一阶段的重点。
