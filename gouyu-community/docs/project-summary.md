...
- 独立 `gouyu` 数据库和 11 张 `gy_*` 业务表已经建立；
...
`PostCommentController` 当前只有领域入口，没有声明可调用方法，因此不计入 30 个可调用路由。
...
| 项目 | 数量 |
| --- | ---: |
| Java 主源码 | 71 个文件，约 3786 行 |
| Java 测试源码 | 3 个文件 |
| 静态 HTML 页面 | 10 个 |
| `web/assets` 本地资源 | 47 个文件 |
...
TTL 为 2 分钟。当前项目没有接入真实短信平台，验证码只写入 Redis 并输出到 DEBUG 日志。
...
会话以 Redis Hash 保存，默认 TTL 为 `36000` 分钟。前端公共 Axios 拦截器从 `sessionStorage` 读取 token，并放入 `authorization` 请求头。

需要特别说明：当前 `MemberServiceImpl.login` 中的验证码比对代码被注释，因此现阶段只校验手机号格式，不校验提交验证码是否等于 Redis 中的验证码。`LoginRequest` 虽包含 `password`，但服务逻辑也没有实现密码校验，所以 `login2.html` 只是复用同一登录接口的页面形式，不是真正完成的密码认证。
...
公开路径包括验证码、登录、热门动态、全部商户路径、商户分类、上传和权益基础路径。其他动态、关注、权益订单、签到和成员详情接口需要登录。
...
Consumer gy-benefit-order-consumer
```
...
最终联调已经验证：接口返回订单 ID，Redis 库存完成扣减，Stream 被消费者处理，MySQL 生成对应订单，数据库库存同步扣减。
...
- 当前数据库没有给 `(member_id, benefit_id)` 和 `(member_id, target_member_id)` 建联合唯一约束，重复控制主要依赖业务逻辑。
...
| POST | `/merchant` | 否（当前配置） | 新增商户 |
| PUT | `/merchant` | 否（当前配置） | 更新商户并删除详情缓存 |
...
| 方法 | 路径 | 鉴权 | 说明 |
| --- | --- | --- | --- |
| POST | `/benefit` | 否（当前配置） | 新增普通权益 |
| POST | `/benefit/limited` | 否（当前配置） | 新增限时权益和 Redis 库存 |
...
| POST | `/upload/post` | 否（当前配置） | 上传动态图片 |
| GET | `/upload/post/delete` | 否（当前配置） | 删除上传图片 |
...
| `login.html` | 手机号验证码登录 |
| `login2.html` | 密码登录样式页，当前仍复用统一登录接口 |
...
| `info.html` | 当前成员主页、自己的动态、关注流和登出 |
| `other-info.html` | 其他成员主页、动态、关注状态和共同关注 |
| `info-edit.html` | 成员资料编辑界面骨架 |
...
页面使用的品牌标识、默认头像、分类图标、商户图片、动态图片和 UI 图标均保存在 `web/assets`，初始化 SQL 也使用本地资源路径，避免依赖外部图片站点。

### 16.4 当前页面边界

- 评论表和实体存在，但 `PostCommentController` 没有读写接口；动态详情页的部分评论内容属于静态展示。
- 个人资料编辑页面存在，但当前没有成员资料更新 API，不能形成完整保存闭环。
- 密码登录页面存在，但后端尚未实现密码校验。
...
- 验证码发送、成员自动创建、登录、会话读取、登出和旧 token 失效；
- 商户分类、普通分页、GEO 距离分页、详情缓存、新增和更新；
- 普通权益、限时权益、商户权益列表；
- Lua 库存与重复领取校验、Stream 投递、异步订单落库和库存扣减；
- 关注、关注状态、共同关注和取消关注；
...
以下内容是当前源码的真实状态。它们不影响本次 38 个场景通过，但如果准备生产部署，应优先处理。

### 21.1 认证与权限

- 验证码比对被注释，任意验证码均可登录合法手机号。
- 密码登录没有实现密码校验。
- 验证码只输出日志，没有接入短信服务。
- `/merchant/**`、`/benefit/**` 和 `/upload/**` 被整体匿名放行，其中包含新增、更新和删除类操作。
- 没有管理员、商户或运营角色，无法限制管理类接口。
- 会话 TTL 为 36000 分钟，周期较长，应根据安全要求重新评估。
- 没有登录频率限制、验证码发送频率限制、设备管理、风控和审计日志。

### 21.2 数据一致性

- 点赞采用先更新 MySQL、再更新 Redis ZSet，两步不在同一事务中，异常时可能不一致。
- 关注采用先更新 MySQL、再更新 Redis Set，也存在双写一致性窗口。
- 共同关注完全依赖 Redis Set，但当前没有从 MySQL 自动重建关注集合的机制。
- 新增商户不会自动写入 GEO；修改坐标也不会同步更新 GEO。
- 新增商户接口本身不主动处理详情缓存，仅更新接口删除缓存。
- 数据库缺少部分联合唯一约束，最终一致性较依赖应用层检查。

### 21.3 权益订单可靠性

- 当前活跃 Lua 只检查 Redis 库存和重复领取，没有校验 `starts_at`、`ends_at`；时间窗口需要在进入 Lua 前或 Lua 可访问的数据中补充。
- Stream 和消费组需要外部初始化；不存在时消费者会持续报错。
- 消费者使用固定消费者名，多实例部署时会产生消费者身份冲突。
- `proxy` 在首次领取请求中通过 `AopContext.currentProxy()` 初始化；如果应用启动时先处理历史 Pending 消息，可能在代理初始化前调用。
- API 在消息落库前就返回订单 ID，客户端没有订单状态查询接口来确认最终成功或失败。
...
- 热门动态和详情会逐条查询作者，存在 N+1 查询。
- 点赞成员读取 `ZSET range 0..4`，得到的是时间戳较小的前 5 位成员，更接近“最早点赞成员”，不是按热度定义的 Top 5。
- Feed 使用写扩散，普通成员合适，但大 V 粉丝量很大时发布成本会显著增加。

### 21.5 文件上传

- 上传接口没有校验文件 MIME、扩展名、大小和图片内容。
- 删除接口接收客户端文件名，只有目录判断，缺少规范化路径和根目录越界校验。
- 上传接口匿名开放，可能被滥用。
- 前端 `post-edit.html` 把后端返回路径拼接为 `/imgs...`，而当前 Nginx 静态根目录和实际上传目录使用 `web/assets`；上传 API 本身已联调通过，但页面展示路径仍应统一为 `/assets/posts/...` 或增加明确的 `/imgs` 映射。
...
1. 恢复并完善验证码校验，接入短信服务和频率限制。
2. 建立角色与权限模型，保护商户、权益和上传写接口。
3. 修复上传文件校验、路径穿越和前端访问路径问题。
4. 在权益领取链路加入开始/结束时间判断，并提供订单状态查询。
5. 自动创建 Stream/消费组，修复消费者代理初始化和多实例消费者命名。
6. 增加数据库联合唯一约束，至少覆盖成员-权益和成员-关注关系。
7. 为敏感配置接入安全配置中心或密钥管理，不只依赖本机环境变量。
...
1. 实现评论读写、评论点赞和回复。
2. 实现成员资料编辑、头像上传和粉丝/关注列表。
3. 建设商户与运营管理后台。
4. 增加权益记录查询、使用、过期和退款状态流转。
5. 完善搜索、评价和首页推荐。
...
- **质量闭环**：编译、打包、页面、Nginx、MySQL、Redis 和 38 场景联调；
...
从“项目包装与独立交付”的角度看，当前改造目标已经完成；从“生产系统”的角度看，项目仍处在可运行原型和高质量教学工程之间，认证权限、文件安全、数据一致性、权益可靠性、评论与资料功能、自动化测试和部署体系是下一阶段的重点。
