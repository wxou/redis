...
| MySQL 构域库 | 通过 | 本机 MySQL 8.0.45；独立 `gouyu` 库 12 张业务表和 1 张认证审计表 |
...
- 主源码 94 个、测试源码 5 个、MySQL 表 13 张、可调用路由仍为 35 个。
- Redis 6.2.6 上 6 项权益 Lua/Stream 测试全部通过。
- 正常落库、确定失败补偿、跨消费者 Pending 接管、毒消息死信、DLQ 对账和状态 DTO 均通过真实环境验证。
- 成功、补偿和死信终态后主 Stream/Pending 均清零；长期历史由 MySQL 处理表保留。
- Nginx 页面、`/api` 代理和权益 `PENDING -> SUCCESS` 轮询链路通过。
