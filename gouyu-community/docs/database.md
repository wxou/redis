...
| `gy_benefit_order_process` | 权益 | Stream 消息状态、处理次数、错误、Payload 与终态历史 |
| `gy_post` | 内容 | 社区动态 |
...
- `gy_benefit_order_process.stream_record_id` 和非空 `order_id` 唯一，保留消息处理终态并支撑幂等查询。
...
应用默认在启动时复用现有数据源执行 `CREATE TABLE IF NOT EXISTS`，非破坏性补齐认证审计表和权益订单处理表。生产环境如统一由 DBA 或迁移系统管理表结构，可关闭对应初始化开关，并手工执行 `20260718_auth_hardening.sql` 与 `20260722_benefit_order_reliability.sql`。迁移不删除现有业务数据，可重复执行。
...
- `gy:limited-benefit:meta:{id}` 与 `gy:limited-benefit:stock:{id}`：应用启动时从 `gy_limited_benefit` 补齐缺失状态。
